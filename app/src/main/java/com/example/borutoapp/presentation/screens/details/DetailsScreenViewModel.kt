...
class DetailsScreenViewModel @Inject constructor(
    private val useCase: UseCases, savedStateHandle: SavedStateHandle
) : ViewModel() {
    ...
    init {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            _selectedHero.value = heroId?.let { useCase.getSelectedHeroUseCase.invoke(it) }
        }
    }
    ...
    fun generateColorPalette() {
        viewModelScope.launch {
            _uiEvent.emit(UiEvents.GenerateColorPalette)
        }
    }
...
sealed class UiEvents {
    data object GenerateColorPalette : UiEvents()
}